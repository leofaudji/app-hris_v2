...
protected $CurRotation = 0;    // current rotation
...
function Write($h, $txt, $link='')
{
	...
	while($i<$nb)
	{
		...
		{
			// Explicit line break
			$this->Cell($w,$h,substr($s,$j,$i-$j),0,2,'',$this->ColorFlag,$link);
			$i++;
			$sep = -1;
			$j = $i;
			$l = 0;
		...
		if($l>$wmax)
		{
			...
			{
				$this->Cell($w,$h,substr($s,$j,$sep-$j),0,2,'',$this->ColorFlag,$link);
				$i = $sep+1;
			}
			$sep = -1;
			$j = $i;
	...
	// Last chunk
	if($i!=$j)
		$this->Cell($l/1000*$this->FontSize,$h,substr($s,$j),0,0,'',$this->ColorFlag,$link);
}
...
function _beginpage($orientation, $size, $rotation)
{
	...
	if($orientation!=$this->DefOrientation)
		$this->CurPageFormat = array('w'=>$this->hPt, 'h'=>$this->wPt);
	else
...
function _mapstring($s)
{
	// Remap characters in a string
	$map = $this->CurrentFont['MAP'];
	$nb = strlen($s);
	$s2 = '';
	for($i=0;$i<$nb;$i++)
	{
		$c = $s[$i];
		$s2 .= isset($map[$c]) ? $map[$c] : $c;
	}
	return $s2;
}
...
function _readpngstream($f, $file)
{
	// Read a PNG file stream
	// Check signature
...
function _putfonts()
{
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		elseif($type=='Type1' || $type=='TrueType')
		{
			...
			{
				if(isset($font['diff']))
					$this->_out('/Encoding <</Type /Encoding /BaseEncoding /WinAnsiEncoding /Differences ['.($this->n+3).' 0 R]>>');
				else
					$this->_out('/Encoding /WinAnsiEncoding');
			}
			$this->_out('>>');
		...
		else
		{
			...
			$this->_out('/Subtype /'.$type);
			$this->_out('/BaseFont /'.$name);
			$this->_out('/FirstChar 0 /LastChar 255');
			$this->_out('/Widths '.($this->n+1).' 0 R');
			$this->_out('/FontDescriptor '.($this->n+2).' 0 R');
			if(isset($font['MAP']))
...
function _putimages()
{
	foreach($this->images as $file=>$info)
	{
		...
		if(isset($info['smask']))
		{
			...
			$this->_out('/Filter /FlateDecode');
			$this->_out('/DecodeParms <</Predictor 15 /Colors 1 /BitsPerComponent 8 /Columns '.$info['w'].'>>');
...
function _putstreamobject($s)
{
	if($this->compress)
	{
		...
		$filter = '';
	$this->_newobj();
	$this->_out('<<'.$filter.'/Length '.strlen($s).'>>');
	$this->_putstream($s);
	$this->_out('endobj');
}
}
